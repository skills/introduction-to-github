...
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
  <path d="M10 3L8 8l-5 2 5 2 2 5 2-5 5-2-5-2-2-5zM21 12l-2 2-2-2-2 2 2 2-2 2 2 2 2-2 2 2-2-2 2-2z"/>
</svg>
`;
...
const InvoiceForm = ({ docType, invoice, children }) => {
    ...
    const handleGenerateDescription = useCallback(async (id, keywords) => {
        if (!keywords || !keywords.trim()) {
            alert('Please enter some keywords to generate a description from.');
            return;
        }
        setGenerating(prev => ({ ...prev, [id]: true }));
        try {
            const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
            const prompt = `You are an assistant for a jewelry store. Based on the keywords "${keywords}", generate a concise and professional product name for a bill. Return only the name itself, nothing else. For example, for "gold ring 22k 8g", a good name would be "22k Gold Ring (8g)". For "silver leg chain", a good name would be "Silver Anklet".`;
            ...
            const newName = response.text.trim().replace(/^"|"$/g, '');
            if (newName) {
            ...
            alert('Could not generate item description. Please check the console for details.');
        } finally {
        ...
        <tbody>
          ${calculatedItems.map((item) => html`
            <tr key=${item.id}>
              <td>
                <div class="name-cell">
                    <input type="text" value=${item.name} onInput=${(e) => handleItemChange(item.id, "name", e.currentTarget.value)} placeholder="Item keywords..." disabled=${generating[item.id]} />
                    <button 
                        class="btn-ai-generate" 
                        onClick=${() => handleGenerateDescription(item.id, item.name)} 
                        disabled=${generating[item.id] || !item.name}
                        aria-label="Generate item description with AI"
                        title="Generate with AI"
                    >
                        ${generating[item.id] ? html`<div class="spinner"></div>` : html`<${SparkleIcon} />`}
                    </button>
                </div>
              </td>
